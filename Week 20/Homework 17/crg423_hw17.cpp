...
void RBT<T>::singleCR(RBTNode<T> *&point) {
    // if point or left child == nullptr -> cannot rotate right
    if (!point || !point->left) return;

    RBTNode<T> *grandparent = point;
    RBTNode<T> *parent = point->left;
    // TODO: ADD ROTATION CODE HERE

    // move parent's right subtree to grandparent's left
    ...
    // if grandparent was the root -> update root
    if (!grandparent->parent) 
        root = parent;

    // if grandparent was a left child -> update left 
    else if (grandparent == grandparent->parent->left)
        grandparent->parent->left = parent;

    // if grandparent was a right child -> update right 
    else 
        grandparent->parent->right = parent;

    // reattach grandparent as right child of parent
...
void RBT<T>::singleCCR(RBTNode<T> *&point) {
    // if point or right child == nullptr -> cannot rotate left
    if (!point || !point->right) return;

    RBTNode<T> *grandparent = point;
    RBTNode<T> *parent = point->right;
    // TODO: ADD ROTATION CODE HERE

    // move parent's left subtree to grandparent's right
    ...
    // reattach grandparent as left child of parent
...
void RBT<T>::fixInsert(RBTNode<T> *&point)
{
    ...
    // while the parent != nullptr and is red
    while (getColor(point->parent) == RED) {
        RBTNode<T>* grandparent = point->parent->parent;

        // if there is no grandparent -> break loop
        ...
        if (point->parent == grandparent->left) {
            ...
            // if uncle != nullptr and is red -> recolor
            if (getColor(uncle) == RED) {
                swapColor(point->parent);  // change parent to black
                swapColor(uncle);          // change uncle to black
                swapColor(grandparent);    // change grandparent to red
                point = grandparent;       // move up to check grandparent
            } 
            else {
                // if uncle is black and point is a right child -> counterclockwise rotation
                if (point == point->parent->right) {
                    point = point->parent;  // move up to parent
                    singleCCR(point);
                }
